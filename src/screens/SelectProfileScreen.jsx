...
const SelectProfileScreen = ({ navigation }) => {
  ...
  console.log(
    "User logged (SelectProfileScreen) :",
  ...
  const handleAddProfile = async () => {
    ...
    if (
      ...
      if (!imageID) {
        uploadedImageURI =
          "https://uobdeuywixmstdrxugmh.supabase.co/storage/v1/object/public/ProfilePictures/" +
          user.uid.slice(0, 10) +
          newProfileName +
          ".jpeg";
      ...
      Alert.alert(
        "Error",
        "Please fill all fields, including selecting a role."
      );
    }
  };
  ...
  const [profileAttempt, setProfileAttempt] = useState();

  const handleVerifyPasskey = () => {
    if (passkeyAttempt === profileAttempt.passkey) {
      dispatch(setSelectedProfileId(profileAttempt.id));
  ...
  const handleSelectProfile = () => {
    ...
    console.log(passkeyModal, "PASSKEY MODAL");
  };

  return (
    <BackgroundImage
      source={require("../assets/backgrounds/pattern_3.png")}
      resizeMode="cover"
    ...
    >
      ...
      >
        ...
        <FlatList
          ...
          renderItem={({ item }) => {
            const profileImage = item.imageID
              ? avatarImages[item.imageID]
              : { uri: item.avatarURI };
            return (
              <TouchableOpacity
                onPress={() => {
                  handleSelectProfile(item.id);
                  setProfileAttempt(item);
                }}
                style={[
        ...
        {addingProfile && (
          <Animated.View
            style={[styles.addProfileContainer, { opacity: fadeAnim }]}
          >
            ...
            <View style={styles.roleSelectionContainer}>
              <View style={styles.roleCheckbox}>
                <BouncyCheckbox
                  ...
                  unfillColor="#E4F1F4"
                  onPress={(isChecked) =>
                    handleRoleSelect(isChecked ? "child" : "parent")
                  }
        ...
        {passkeyModal && (
          <Modal
            visible={passkeyModal}
            transparent={true}
            animationType="slide"
          >
            <View style={styles.passkeyModal}>
              <View style={styles.passkeyContainer}>
                <TextInput
                  value={passkeyAttempt}
                  onChangeText={(pass) => setPasskeyAttempt(pass)}
                  placeholder="Enter a 4-digit code"
                  placeholderTextColor="#AAA"
                  keyboardType="numeric"
                  maxLength={4}
